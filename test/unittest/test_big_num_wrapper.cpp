...
TEST_CASE( "write_big_int_term" ) {
    std::string str = "x + ";
    fmpz big_num = 0;

    SECTION("Basic"){
        big_num = 42;
        write_big_int_term(str, big_num);
        REQUIRE(str == "x + 42");
    }
...
TEST_CASE( "write_big_rational_term" ) {
    std::string str = "x + ";
    fmpq_t big_num;
    fmpq_init(big_num);
    fmpz* num = fmpq_numref(big_num);
    ...
    SECTION("plaintext basic"){
        fmpq_set_ui(big_num, 3, 2);
        write_big_rational_term<PLAINTEXT_OUTPUT>(str, *big_num);
        REQUIRE(str == "x + 3/2");
    }

    SECTION("typeset basic"){
        fmpq_set_ui(big_num, 3, 2);
        write_big_rational_term<TYPESET_OUTPUT>(str, *big_num);
        REQUIRE(str == "x + ⁜f⏴3⏵⏴2⏵");
    }
